...
class MyTasksDetailsScreen extends Component {
  ...
  async setImageForFile(res) {
    ...
    if (fileSize <= 10) {
      ...
      this.uploadFile(this.state.files, this.state.selectedTaskID);
    } else {
      Utils.showAlert(
        true,
        '',
  ...
  async uploadFile(files, selectedTaskID) {
    await APIServices.addFileToMyTaskData(files, selectedTaskID)
      .then(response => {
